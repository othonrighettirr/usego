...
let MessagesService = MessagesService_1 = class MessagesService {
    ...
    async getValidWhatsAppNumber(socket, phone) {
        ...
        throw new common_1.BadRequestException(`Número ${originalNumber} não encontrado no WhatsApp. ` +
    ...
    async sendImage(dto) {
        ...
        try {
            ...
            try {
                ...
                throw new common_1.BadRequestException(`Não foi possível enviar a imagem. Verifique se a URL é acessível: ${imageUrl}`);
            }
        }
    ...
    async sendVideo(dto) {
        ...
        try {
            ...
            try {
                ...
                throw new common_1.BadRequestException(`Não foi possível enviar o vídeo. Verifique se a URL é acessível: ${videoUrl}`);
            }
        }
    ...
    async sendDocument(dto) {
        ...
        try {
            ...
            try {
                ...
                throw new common_1.BadRequestException(`Não foi possível enviar o documento. Verifique se a URL é acessível: ${documentUrl}`);
            }
        }
...
END:VCARD`;
    ...
    async getNewsletterMetadata(instanceId, newsletterId) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
        try {
            ...
            if (!metadata) {
                throw new common_1.BadRequestException('Newsletter não encontrada');
            }
            return {
                success: true,
                newsletter: metadata
            ...
            throw new common_1.BadRequestException(`Newsletter não encontrada ou inacessível: ${error.message}`);
        }
    }
    async createNewsletter(instanceId, name, description) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
        try {
            ...
            throw new common_1.BadRequestException(`Erro ao criar newsletter: ${error.message}`);
        }
    }
    async followNewsletter(instanceId, newsletterId) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
        const newsletterJid = newsletterId.includes('@newsletter')
        ...
        try {
            ...
            throw new common_1.BadRequestException(`Erro ao seguir newsletter: ${error.message}`);
        }
    }
    async unfollowNewsletter(instanceId, newsletterId) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
        const newsletterJid = newsletterId.includes('@newsletter')
        ...
        try {
            ...
            throw new common_1.BadRequestException(`Erro ao deixar de seguir newsletter: ${error.message}`);
        }
    }
    async muteNewsletter(instanceId, newsletterId) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
        const newsletterJid = newsletterId.includes('@newsletter')
        ...
        try {
            ...
            throw new common_1.BadRequestException(`Erro ao silenciar newsletter: ${error.message}`);
        }
    }
    async unmuteNewsletter(instanceId, newsletterId) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
        const newsletterJid = newsletterId.includes('@newsletter')
        ...
        try {
            ...
            throw new common_1.BadRequestException(`Erro ao dessilenciar newsletter: ${error.message}`);
        }
    }
    async getNewsletterSubscribers(instanceId, newsletterId) {
        ...
        try {
            ...
            throw new common_1.BadRequestException(`Erro ao obter inscritos: ${error.message}`);
        }
    }
    async getNewsletterMessages(instanceId, newsletterId, count = 10) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
        const newsletterJid = newsletterId.includes('@newsletter')
        ...
        try {
            ...
            throw new common_1.BadRequestException(`Erro ao buscar mensagens: ${error.message}`);
        }
    }
    ...
    async getGroupParticipants(instanceId, groupId) {
        ...
        try {
            ...
            throw new common_1.BadRequestException(`Erro ao buscar participantes: ${error.message}`);
        }
    }
    ...
    async getFollowedNewsletters(instanceId) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
    ...
    async getChannelSubscribers(instanceId, newsletterId) {
        const socket = this.baileys.getSocket(instanceId);
        if (!socket)
            throw new common_1.BadRequestException('Instância não conectada');
        try {
